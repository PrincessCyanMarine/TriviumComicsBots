...
export function testCommands(msg: Message) {
    ...
    if (testWord(args, "dumbass") && randomchance()) {
        //say(krystal, msg.channel, userMention("601943025253482496"));
    }
    if (testWord(args, "arson")) {
        // burning(msg);
    } else if (testWord(args, "ye{2,}t")) yeeting(msg);
    else if (testWord(args, "pf{2,}t")) {
        // rebel(msg, true);
    } else if (args == "18?") {
        // eighteen(msg);
        ...
        else if (testWord(args, "silence") && msg.mentions.members?.first()) {
            if (randomchance(50)) killing(msg);
            else silencing(msg);
