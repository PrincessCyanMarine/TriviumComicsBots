...
Object.values(clients).forEach((bot) => {
    bot.on("interactionCreate", (async (interaction) => {
        ...
        if (challeging && challeging.groups && interaction.channel?.isTextBased()) {
        ...
        if (interaction.customId.includes("rpssp")) return handleSelectMenu(interaction);
            ...
            if (interaction.user.id != interaction.customId.split("Remove-")[1])
                return interaction.reply({ content: "You can't do that", ephemeral: true });
            ...
            interaction.update({
                ...
                // TODO SEE IF WORKS
                components: [
                    new ActionRowBuilder().addComponents(get_rps_interactible(interaction.user.id, interaction.customId.startsWith("a"))) as any,
                ],
                embeds: [],
            });
        }
    }) as (interaction: Interaction) => Promise<void>);
});

export function playrps(bot_name: string, id: string, channel: TextBasedChannel, list: boolean = false, wrongplayer: boolean = false) {
    bot_name = bot_name.toLowerCase();
    if (bot_name == "random") bot_name = random_from_array(rps_bots.slice(0, rps_bots.length - 1));
    ...
    say(
        ...
        channel,
        {
            ...
            components: [new ActionRowBuilder().addComponents(method)],
        },
        100
    );
}
...
async function handleSelectMenu(interaction: SelectMenuInteraction | ButtonInteraction) {
    ...
    if (interaction.customId.includes("rpssp")) {
        ...
        let choice_name;
        if (interaction instanceof SelectMenuInteraction) choice_name = interaction.values[0];
        else choice_name = interaction.customId.split("/")[1];
        ...
        if (interaction.message.author.id == krystalId) {
            let embed = new EmbedBuilder()
                .setAuthor({
                    name: "The Cosmic D20",
                    iconURL: "https://cdn.discordapp.com/avatars/743606862578057277/86f4b6b4075938799f679e80f75634ab.png?size=1024",
                })
                .addFields([
                    {
                        name:
                            interaction.member && interaction.member instanceof GuildMember
                                ? interaction.member.displayName
                                : interaction.user.username,
                        value: choice_name,
                    },
                    { name: "Krystal", value: ["popcorn...?", "gun...?", "fireball...?"][pc] },
                    {
                        name: "Result",
                        value: "Krystal wins... Somehow...",
                    },
                ])
                .setColor(Colors.Red);
            ...
            let file = [() => eating(undefined, interaction.user), () => killing(undefined, interaction.user), () => burning(undefined)][pc];
        ...
        let components = interaction.message.components?.map((c) => (c instanceof ActionRowBuilder ? c : new ActionRowBuilder()));
        ...
        let embed = new EmbedBuilder()
            ...
            .addFields([
                {
                    name:
                        interaction.member && interaction.member instanceof GuildMember ? interaction.member.displayName : interaction.user.username,
                    value: choice_name,
                },
                {
                    name: bot_name,
                    value: ["rock", "paper", "scissors"][pc],
                },
                {
                    name: "Result",
                    value:
            ...
            .setColor(result == "DRAW" ? Colors.Yellow : result == "PLAYER_WON" ? Colors.Green : Colors.Red);

        interaction.update({
            embeds: [embed],
            content: rockpaperscissors_messages[result == "DRAW" ? "PLAYER_WON" : result][bot_name] + "\nPlay again?",
...
function sendtoplayer(interaction: ButtonInteraction | SelectMenuInteraction, text: string) {
    ...
    interaction.followUp({
        content: text,
        ephemeral: true,
        components: [new ActionRowBuilder().addComponents(rps_interactible) as any],
    });
}
