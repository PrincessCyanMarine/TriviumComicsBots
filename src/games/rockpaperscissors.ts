...
Object.values(clients).forEach((bot) => {
  bot.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.customId.includes("rpssp")) return handleSelectMenu(interaction);
      ...
      interaction.update({
        content: text,
        components: [new MessageActionRow().addComponents(get_rps_interactible(interaction.user.id, interaction.customId.startsWith("a")))],
        embeds: [],
      });
    }
  });
...
function sendtoplayer(interaction: ButtonInteraction | SelectMenuInteraction, text: string) {
  ...
  interaction.followUp({
    content: text,
    ephemeral: true,
    components: [new MessageActionRow().addComponents(rps_interactible)],
  });
}
