  ...
  if (args.startsWith("!")) {
    ...
    switch (args.split(" ")[0]) {
      ...
      case "play_list":
        ...
        if (!list) {
          rps_bots.forEach((bot) => {
            if (components.length == 0 || components[components.length - 1].components.length == 5) components.push(new MessageActionRow());
      ...
      case "activity":
        ...
        let act =
          options[2] && parseInt(options[2]) < acts[options[1]].length ? acts[options[1]][parseInt(options[2])] : random_from_array(acts[options[1]]);
        changeActivity(...act);
        break;
      case "bird":
      case "birds":
