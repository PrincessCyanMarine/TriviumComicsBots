import { Message, GuildMember, ButtonStyle, ActionRowBuilder, ActionRow } from "discord.js";
...
function nitro(msg: Message) {
    mod_alert_webhook(testing).send({
        ...
        embeds: msg2embed(msg),
        // TODO SEE IF WORKS
        components: [new ActionRowBuilder().addComponents(button_message_link(msg.url)) as any],
    });

    if (msg.member && msg.member instanceof GuildMember && msg.guild) warn(msg.member, msg.guild.id, "Possible free nitro scam", msg.channel);
}

const button_message_link = (url: string) => new ButtonBuilder().setStyle(ButtonStyle.Link).setURL(url).setLabel("Go to message");
